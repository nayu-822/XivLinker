...
- `README.md`、`docs/` 配下、各プロジェクト配下の補足ドキュメントは原則として日本語で記述する。
- API 名、クラス名、ライブラリ名、プロトコル名などは必要に応じて英語表記を維持してよい。
- 文書ファイルの文字コードは UTF-8 を使用する。
- 文字化けを避けるため、エディタや生成処理で Shift_JIS など UTF-8 以外を新規採用しない。
- 既存ドキュメントを更新する場合も、文字コードは UTF-8 のまま維持する。
