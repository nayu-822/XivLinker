...
- アプリ内で利用者に表示する文言は原則として日本語を使用する。
- 接続状態、画面見出し、ボタン名、ログ見出し、プレースホルダ、エラーメッセージの要約は日本語を優先する。
- ライブラリ名、プロトコル名、設定キー名、外部サービス名など固有の識別子は必要に応じて英語表記を維持してよい。
- 英語表記を残す場合でも、利用者向け説明文は日本語で補足する。
