...
- Codexは作業完了時の報告に、変更内容の要約に加えてコミットメッセージ案を必ず含める。
- コミットメッセージ案は、今回の変更内容が分かる簡潔な日本語1件を基本とする。

## ドキュメント更新ルール

- 仕様、挙動、構成、手順、前提条件に変更が入った場合は、関連する `README.md` または `docs/` 配下のドキュメントを最新状態に保つ。
- リポジトリ内に明示ルールがない場合でも、Codexはドキュメントを最新に保つ前提で必要な更新有無を確認する。
- ドキュメント更新が不要だった場合も、完了報告でその旨を簡潔に示してよい。
